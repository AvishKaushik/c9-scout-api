"""VALORANT team queries for GRID API.
...
class ValorantTeamQueries:
    """Query builder for VALORANT team data from Central Data API."""
    ...
    async def get_team_by_external_id(
        ...
        data_provider: str,
        external_id: str,
        title_id: str = VALORANT_TITLE_ID,
    ) -> dict[str, Any]:
    ...
    async def get_team_economy(
        self,
        team_id: str,
        match_ids: Optional[list[str]] = None,
    ) -> dict[str, Any]:
        """DEPRECATED: Economy stats not available from Central Data API.

        Use get_team_series() and fetch economy data from Series State API.
        """
        import warnings
        warnings.warn(
            "get_team_economy() is deprecated. Economy stats must be computed from Series State API data.",
            DeprecationWarning,
        )
        return await self.get_team_info(team_id)
