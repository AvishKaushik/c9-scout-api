"""League of Legends player queries for GRID API.
...
class LoLPlayerQueries:
    """Query builder for LoL player data."""
    ...
    # Central Data API: Get recent series for a player (LoL title ID = 3)
    ...
    async def get_player_info(self, player_id: str) -> dict[str, Any]:
        ...
        return await self.client.execute(
            ...
            cache_key=f"lol_player_{player_id}",
        )
    ...
    async def get_player_series(
        ...
        """
        # Default to LoL title ID if not specified
    ...
    async def get_champion_mastery(self, player_id: str) -> dict[str, Any]:
        """Get player's champion pool and mastery data."""
        # Fetch recent performances and aggregate by champion
        ...
        for perf in performances:
            ...
            if champ_id not in champion_stats:
                champion_stats[champ_id] = {
                    "champion": {"id": champ_id, "name": champ_name},
                    "gamesPlayed": 0,
                    "wins": 0,
                    "losses": 0,
                    "totalKills": 0,
        ...
        return {
            "player": {
                **perf_data.get("player", {}),
                "championMastery": sorted(mastery, key=lambda x: x["gamesPlayed"], reverse=True),
            }
        }
