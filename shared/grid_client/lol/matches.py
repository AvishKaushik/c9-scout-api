"""League of Legends match/series queries for GRID API.

NOTE: In GRID API, matches are called "Series". A Series contains multiple Games.
- Central Data API: Series metadata (teams, tournament, schedule)
- Series State API: Detailed game stats (player performance, events, drafts)
...
class LoLMatchQueries:
    """Query builder for LoL match/series data."""
    ...
    async def get_series_list(
        ...
        """
        # Default to LoL title if not specified
    ...
    async def get_series_state(self, series_id: str) -> dict[str, Any]:
        ...
        - Player stats (kills, deaths, assists, gold)
        - Character/champion picks
        - Team scores
    ...
    async def get_match_timeline(
        ...
        """DEPRECATED: Timeline data available in Series State API.

        Use get_series_state() to get game-by-game data.
        """
        import warnings
        warnings.warn(
            "get_match_timeline() is deprecated. Use get_series_state() instead.",
            DeprecationWarning,
        )
